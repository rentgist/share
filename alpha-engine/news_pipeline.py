...
# 블룸버그와 로이터 등은 공식 무료 RSS 지원이 끊기거나 파싱 방어가 있는 경우가 많아 WSJ, CNBC, FT 등 신뢰도 높은 피드로 보완
RSS_FEEDS = {
    ...
    "Financial_Times_US": "https://www.ft.com/rss/home/us"
}
...
다음은 {source}에서 발췌한 경제/금융 속보의 제목과 요약이다.
이 기사가 주식 시장과 거시 경제(금리, 환율, 유동성 등)에 미칠 구조적 영향을 입체적으로 분석하라.

반드시 유효한 JSON 형식으로만 응답하라.
{{
    "sentiment": "호재" | "악재" | "중립",
    "importance": <1~5 사이의 정수. 거시경제 충격은 4~5, 개별 기업 소식은 1~2>,
    "sectors": ["수혜/피해 예상 섹터명1", "섹터명2"],
    "keywords": ["핵심키워드1", "핵심키워드2"],
    "reason": "이 뉴스가 증시 및 매크로에 미칠 실질적 영향을 1~2줄로 명료하게 요약"
}}
